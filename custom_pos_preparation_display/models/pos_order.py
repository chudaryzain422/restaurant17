...
class PosOrder(models.Model):
    ...
    def create_from_ui(self, orders, draft=False):
        ...
        for order in order_ids:
            if order.state == 'paid':
                self.env['custom_pos_preparation_display.order'].process_order(order.id)
        return orders
    ...
    def _process_preparation_changes(self, cancelled=False, note_history=None,is_split=False):
        ...
        pdis_order = self.env['custom_pos_preparation_display.order'].search(
        ...
        if any([quantities['order'] > quantities['display'] for quantities in quantity_data.values()]):
            flag_change = True
            sound = True
            pdis_ticket = self.env['custom_pos_preparation_display.order'].create({
                'displayed': True,
                'pos_order_id': self.id,
            ...
            if data['order'] > data['display']:
                ...
                for line in filtered_lines:
                    ...
                    if missing_qty == 0 and line_qty > 0:
                        ...
                        self.env['custom_pos_preparation_display.orderline'].create({
                            'todo': True,
