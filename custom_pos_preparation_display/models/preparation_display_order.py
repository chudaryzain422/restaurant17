...
class PosPreparationDisplayOrder(models.Model):
    _name = 'custom_pos_preparation_display.order'
    ...
    order_stage_ids = fields.One2many('custom_pos_preparation_display.order.stage', 'order_id', help="All the stage ids in which the order is placed")
    preparation_display_order_line_ids = fields.One2many(
        'custom_pos_preparation_display.orderline',
    ...
    def _send_orders_to_preparation_display(self, preparation_display_id):
        preparation_display = self.env['custom_pos_preparation_display.display'].browse(preparation_display_id)
    ...
    def change_order_stage(self, stage_id, preparation_display_id):
        ...
        p_dis = self.env['custom_pos_preparation_display.display'].search([('id', '=', preparation_display_id)])
    ...
    def get_preparation_display_order(self, preparation_display_id):
        preparation_display = self.env['custom_pos_preparation_display.display'].browse(preparation_display_id)
        orders = self.env['custom_pos_preparation_display.order'].search(['|', ('pos_config_id', 'in', preparation_display.get_pos_config_ids().ids), ('pos_order_id', '=', False)])
    ...
    def _clean_preparation_data(self):
        orders = self.env['custom_pos_preparation_display.order'].search([('write_date', '<=', fields.Datetime.now() - timedelta(days=1))])
        orders.unlink()
        return True
