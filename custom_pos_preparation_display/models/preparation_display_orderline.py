...
class PosPreparationDisplayOrderline(models.Model):
    _name = 'custom_pos_preparation_display.orderline'
    ...
    def change_line_status(self, status):
        ...
        preparation_displays = self.env['custom_pos_preparation_display.display'].search(['|', ('category_ids', 'in', categories.ids), ('category_ids', '=', False)])
    ...
    def send_stricked_line_to_next_stage(self, preparation_display_id):
        ...
        new_order = self.env['custom_pos_preparation_display.order'].create({
            'displayed': True,
