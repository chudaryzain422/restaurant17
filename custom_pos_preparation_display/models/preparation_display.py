...
from odoo.addons.custom_pos_preparation_display.models.preparation_display_orderline import PosPreparationDisplayOrderline

class PosPreparationDisplay(models.Model):
    _name = 'custom_pos_preparation_display.display'
    ...
    stage_ids = fields.One2many('custom_pos_preparation_display.stage', 'preparation_display_id', string="Stages", default=[
    ...
    def get_preparation_display_data(self):
        return {
            ...
            'orders': self.env["custom_pos_preparation_display.order"].get_preparation_display_order(self.id),
    ...
    def open_reset_wizard(self):
        return {
            ...
            'view_mode': 'form',
            'res_model': 'custom_pos_preparation_display.reset.wizard',
            'target': 'new',
    ...
    def open_ui(self):
        return {
            ...
            'url': '/custom_pos_preparation_display/web?display_id=%d' % self.id,
            'target': 'self',
        }
    ...
    def reset(self):
        for preparation_display in self:
            ...
            orders = self.env['custom_pos_preparation_display.order'].search([('|'), ('pos_order_id', '=', False), ('pos_config_id', 'in', preparation_display.get_pos_config_ids().ids)], limit=1000, order='id desc')
    ...
    def _compute_order_count(self):
        for preparation_display in self:
            ...
            orders = preparation_display.env['custom_pos_preparation_display.order'].search([
            ...
            order_stages = self.env['custom_pos_preparation_display.order.stage'].search([
                ('preparation_display_id', '=', preparation_display.id),
                ('create_date', '>=', fields.Date.today()),
                ('done', '=', True)
            ])
