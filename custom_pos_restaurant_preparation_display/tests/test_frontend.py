...
class TestUi(TestFrontend):
    def test_01_preparation_display_resto(self):
        self.env['custom_pos_preparation_display.display'].create({
        ...
        pdis_order1 = self.env['custom_pos_preparation_display.order'].search([('pos_order_id', '=', order1.id)], limit=1)
        ...
        pdis_order2 = self.env['custom_pos_preparation_display.order'].search([('pos_order_id', '=', order2.id)], limit=1)
        ...
        pdis_order3 = self.env['custom_pos_preparation_display.order'].search([('pos_order_id', '=', order3.id)], limit=1)
