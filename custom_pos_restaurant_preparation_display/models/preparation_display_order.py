...
class PosPreparationDisplayOrder(models.Model):
    _inherit = 'custom_pos_preparation_display.order'
    ...
    def process_order(self, order_id, cancelled=False, note_history=None):
        ...
        if order and order.table_id:
            old_orders = self.env['custom_pos_preparation_display.order'].search([('id', '=', order_id), ('pos_table_id', '!=', order.table_id.id)])
            for o in old_orders:
