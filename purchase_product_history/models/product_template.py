...
class ProductTemplate(models.Model):
    ...
    margin_percentage = fields.Float(string="Margin (%)", help="Enter margin percentage")
    ...
    def _compute_sale_price(self):
        for product in self:
            if product.margin_percentage > 0:
                product.list_price = product.standard_price * (1 + product.margin_percentage / 100)
        ...
        self.env['purchase.template.history.line'].create([
            {
                ...
                'vendor_id': rec.order_id.partner_id.id,
